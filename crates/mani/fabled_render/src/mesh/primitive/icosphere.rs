...
//         let key = ((cantor_encode * (cantor_encode + 1)) << 1) +
// std::cmp::min(p1, p2); // Cantor's pairing function
//
...
//                 let y = (vertices[3 * p1 + 1] + vertices[3 * p2 + 1]) * 0.5;
// // Y                 let z = (vertices[3 * p1 + 2] + vertices[3 * p2 + 2]) *
// 0.5; // Z
//
//                 // Normalize it
...
//         //(0.52573, 0.85065)
//         //let modified_vert = (0.525_73 * ico_sphere.radius, 0.850_65 *
// ico_sphere.radius);         let mut normalized_vertices = vec![
...
//         ];
//
//         // Very fast hash algorithm we don't care or are worried of Dos
// Attack so we will use a fast non-cryptographic algorithm         let mut
// middle_index_cache : rustc_hash::FxHashMap<usize, usize>  =
...
//                 let v3 : usize = tri[2];
//
//                 let a = IcoSphere::middle_point(v1,v2, &mut
// normalized_vertices, &mut middle_index_cache);                 let b =
// IcoSphere::middle_point(v2, v3, &mut normalized_vertices, &mut
// middle_index_cache);                 let c = IcoSphere::middle_point(v3, v1,
// &mut normalized_vertices, &mut middle_index_cache);
//
//
//                 let sub_divide_triangle_chunk = [v1, a, c, v2, b, a, v3, c,
// b, a, b, c];
//
//                 {
//                     let offset = index * 12;
...
//         for (index, chunk) in
// normalized_vertices.chunks_exact(3).enumerate(){
//
//             let inv_pi = 1.0 / std::f32::consts::PI;
//
//             // The Last batch of triangles loop back from high uv to zero
//             // A temporary hack is for the Coordinate (U or V) is to use the
// absolute value of chunk[2] (z).atan2(chunk[0](x)) .             //
// chunk[2].abs().atan2(chunk[0]) * (inv_pi * 0.5) + 0.5, // U             let
// uv = [                 chunk[2].atan2(chunk[0]) * (inv_pi * 0.5) + 0.5, // U
...
//
//         for (vertex,i) in
// normalized_vertices.chunks_exact_mut(3).zip(uvs.chunks_exact(2)){            
// let pos_x = vertex[0];             let pos_y = vertex[1];
//             let pos_z = vertex[2];
//
//             let u = i[0];
//             let v = i[1];
...
//                 position: [pos_x * ico_sphere.radius, pos_y *
// ico_sphere.radius, pos_z * ico_sphere.radius],                 tex_coord: [u,
// v],                 normal: [pos_x, pos_y, pos_z],
//                 tangent: [0.0; 4],
//                 bi_tangent: [0.0; 4]
//             });
//
