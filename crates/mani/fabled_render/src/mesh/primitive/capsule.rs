...
//     ) -> Capsule {
//         // Sanity check for the size of latitude, longitude, and ring to
// prevent         // Latitude from reaching beyond 90, longitude from reaching
// above 180,         // and ring from reaching beyond 5.
//         //
//         // Prevent increasing latitude and longitude intentional or
// unintentional         // to the point where it cause  s extreme repeating
// KiPageFault and         // MmAccessFault resulting in cache thrashing or
// memory can't be         // allocated error.
//
...
//         let vertex_offset_south_equator = vertex_offset_cylinder +
// longs_add_1 * rings; // if ring is zero then vertex_offset_cylinder + 0;
//
//         let vert_offset_south_hemisphere = vertex_offset_south_equator +
// longs_add_1;         let vert_offset_south_polar =
// vert_offset_south_hemisphere + longs_add_1 * half_lats_sub_2;         let
// vert_offset_south_cap = vert_offset_south_polar + longs_add_1;
//
//         // Initialize arrays.
//         let vert_len = vert_offset_south_cap + longitude;
//
//         let mut vs: Vec<Vector3> = vec![Vector3::ZERO; vert_len];
//         let mut vts: Vec<Vector2> = vec![Vector2::ZERO; vert_len];
//         let mut vns: Vec<Vector3> = vec![Vector3::ZERO; vert_len];
//
...
//             half_lats as f32 / (rings_add_1 + latitude) as f32, //
// CapsuleUvProfile::Uniform             0.333_333_34,                       /*
// CapsuleUvProfile::Fixed f32 representation of
//                                                  * 1.0 / 3.0 */
//         ];
//
...
//
//         let mut theta_cartesian: Vec<Vector2> = vec![Vector2::ZERO;
// longitude];         let mut rho_theta_cartesian: Vec<Vector2> =
// vec![Vector2::ZERO; longitude];
//
...
//
//             let theta_cartesian_vec2 = Vector2::set(cos_theta, sin_theta);
//
//             theta_cartesian[j] = theta_cartesian_vec2;
//             rho_theta_cartesian[j] = theta_cartesian_vec2 *
// Vector2::splat(radius);
//
//             // North.
//             let north_summit_vec3 = Vector3::set(0.0, summit, 0.0);
//             let tex_polar_vec2 = Vector2::set(s_texture_polar, 1.0);
//
//             vs[j] = north_summit_vec3;
//             vts[j] = tex_polar_vec2;
//             vns[j] = Vector3::UP;
//
//             // South.
//             let idx = vert_offset_south_cap + j;
//             vs[idx] = -north_summit_vec3;
//             vts[idx] = tex_polar_vec2 - Vector2::DOWN;
//             vns[idx] = Vector3::DOWN;
//         }
//
//         // Equatorial vertices.
...
//             vs[index_n] = Vector3::set(rtc.value[0], half_depth,
// -rtc.value[1]);             vts[index_n] = Vector2::set(s_texture,
// vt_aspect_north);             vns[index_n] = Vector3::set(tc.value[0], 0.0,
// -tc.value[1]);
//
//             // South equator.
//             let index_s = vertex_offset_south_equator + j;
//             vs[index_s] = Vector3::set(rtc.value[0], -half_depth,
// -rtc.value[1]);             vts[index_s] = Vector2::set(s_texture,
// vt_aspect_south);             vns[index_s] = vns[index_n];
//         }
//
//         // Hemisphere vertices.
...
//             let vert_curr_lat_north = vert_offset_north_hemisphere +
// i_lon_add_1;             let vert_curr_lat_south =
// vert_offset_south_hemisphere + i_lon_add_1;
//
...
//                 vs[index_n] = Vector3::set(
//                     rho_cos_phi_north * tc.value[0],
//                     z_offset_north,
//                     -rho_cos_phi_north * tc.value[1],
//                 );
//                 vts[index_n] = Vector2::set(s_texture, t_tex_north);
//                 vns[index_n] = Vector3::set(
//                     cos_phi_north * tc.value[0],
//                     -sin_phi_north,
//                     -cos_phi_north * tc.value[1],
//                 );
//
//                 // South Hemisphere
//                 let index_s = vert_curr_lat_south + j;
//                 vs[index_s] = Vector3::set(
//                     rho_cos_phi_south * tc.value[0],
//                     z_offset_south,
//                     -rho_cos_phi_south * tc.value[1],
//                 );
//                 vts[index_s] = Vector2::set(s_texture, t_tex_south);
//                 vns[index_s] = Vector3::set(
//                     cos_phi_south * tc.value[0],
//                     -sin_phi_south,
//                     -cos_phi_south * tc.value[1],
//                 );
//             }
//         }
//
...
//             let t_texture = cmp_l_fac * vt_aspect_north + fac *
// vt_aspect_south;             let z = half_depth - depth * fac;
//
//             for (j, &cache) in s_texture_cache.iter().enumerate() {
//                 let j_mod = j % longitude;
//
...
//                 let rtc = rho_theta_cartesian[j_mod];
//
//                 vs[idx_cyl_lat] = Vector3::set(rtc.value[0], z,
// -rtc.value[1]);                 vts[idx_cyl_lat] = Vector2::set(s_texture,
// t_texture);                 vns[idx_cyl_lat] = Vector3::set(tc.value[0], 0.0,
// -tc.value[1]);
//
//                 idx_cyl_lat += 1;
//             }
...
//         let tri_offset_cylinder = tri_offset_north_hemisphere +
// hemisphere_lon;         let tri_offset_south_hemisphere = tri_offset_cylinder
// + rings_add_1 * lon_mul_6;         let tri_offset_south_cap =
// tri_offset_south_hemisphere + hemisphere_lon;
//
...
//
//             let vert_curr_lat_north = vert_offset_north_hemisphere +
// i_lon_add_1;             let vert_next_lat_north = vert_curr_lat_north +
// longs_add_1;
//
//             let vert_curr_lat_south = vertex_offset_south_equator +
// i_lon_add_1;             let vert_next_lat_south = vert_curr_lat_south +
// longs_add_1;             let mut j = 0;
//             while j < longitude {
//                 // North.
...
//         for i in 0..vert_len {
//             vertices[i] = Vertex {
//                 position: vs[i].to_primitive(),
//                 tex_coord: vts[i].to_primitive(),
//                 normal: vns[i].to_primitive(),
//                 tangent: [0.0; 4],
//                 bi_tangent: [0.0; 4],
//             };
//         }
