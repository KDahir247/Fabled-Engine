pub struct Light {
    ...
    angle: f32,
    //how far away the light will reach. (attenuation constant). If lighting is directional light then range is disregarded
    range: f32,
    //used to modulate the computed light contribution. 1.0 by default.
    intensity: f32,
    //check if light is enabled. if false then after the light culling the light will not show even though it hasn't been culled
    enabled: bool,
