...
impl DualNumber {
    ...
    pub const fn from_primitive(array: [f32; 2]) -> DualNumber {
        DualNumber {
            value: Vector2::from_primitive(array),
        }
    }
}
...
impl Div for DualNumber {
    ...
    fn div(self, rhs: Self) -> Self::Output {
        DualNumber::set(
            ...
            ((self.value.y() * rhs.value.x()) - (self.value.x() * rhs.value.y()))
                / (rhs.value.x() * rhs.value.x()),
        )
    }
}
