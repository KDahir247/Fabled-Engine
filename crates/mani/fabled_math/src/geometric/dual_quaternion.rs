use crate::{DualNumber, Quaternion, Vector3};
...
impl DualQuaternion {
    pub const IDENTITY: DualQuaternion = DualQuaternion {
        real: Quaternion::IDENTITY,
...
impl DualQuaternion {
    ...
    pub const fn set(
        ...
        let real_quaternion =
            Quaternion::set(real_img.x(), real_img.y(), real_img.z(), real_scalar);
        let dual_quaternion =
            Quaternion::set(dual_img.x(), dual_img.y(), dual_img.z(), dual_scalar);

        DualQuaternion {
            real: real_quaternion,
            dual: dual_quaternion,
        }
    }
    ...
    pub const fn broadcast(real_val: f32, dual_val: f32) -> DualQuaternion {
        DualQuaternion {
            real: Quaternion::broadcast(real_val),
            dual: Quaternion::broadcast(dual_val),
        }
    }
    ...
    pub const fn from_primitive(val: [f32; 8]) -> DualQuaternion {
        ...
        DualQuaternion {
            real: Quaternion::from_primitive(real_primitive),
            dual: Quaternion::from_primitive(dual_primitive),
        }
    }
}
...
pub mod dual_quaternion_math {
    ...
    pub fn conjugate_dual_quat(dual_quaternion: DualQuaternion) -> DualQuaternion {
        DualQuaternion {
            real: conjugate_quat(dual_quaternion.real),
            dual: conjugate_quat(dual_quaternion.dual),
        }
    }
    ...
    pub fn dual_number_conjugate_dual_quat(dual_quaternion: DualQuaternion) -> DualQuaternion {
        DualQuaternion {
            real: dual_quaternion.real,
            dual: -dual_quaternion.dual,
        }
    }
    ...
    #[inline]
    pub fn from_translation_rotation_dual_quat(
        translation: Vector3,
        rotation: Quaternion,
    ) -> DualQuaternion {
        let dual_quaternion =
            Quaternion::set(translation.x(), translation.y(), translation.z(), 0.0);
    ...
    pub fn from_rotation_dual_quat(normalized_axis: Vector3, angle_radian: f32) -> DualQuaternion {
        ...
        DualQuaternion {
            real: Quaternion {
                value: (extended_axis * theta_vector).value,
            },
            dual: Quaternion::ZERO,
        }
    }
    ...
    pub fn get_translation_dual_quat(dual_quaternion: DualQuaternion) -> Vector3 {
        ...
        let translation_vec =
            dual_quaternion.dual.scale_quaternion(TWO_VEC) * conjugate_quat(dual_quaternion.real);
    ...
    pub fn length_dual_quat(dual_quaternion: DualQuaternion) -> DualNumber {
        ...
        DualNumber::set(
            real,
            ((dual_quaternion.real.to_real() * dual_quaternion.dual.to_real())
                + dot(real_pure.value, dual_pure.value))
                / real,
        )
    }
    ...
    pub fn length_sqr_dual_quat(dual_quaternion: DualQuaternion) -> DualNumber {
        ...
        let dual_pure = dual_quaternion.dual.to_pure();
        DualNumber::set(
            length_squared(dual_quaternion.real.value),
            2.0 * (dual_quaternion.real.to_real() * dual_quaternion.dual.to_real()
                + dot(real_pure.value, dual_pure.value)),
        )
    }
    ...
    #[inline]
    pub fn transform_point3_dual_quat(
        dual_quaternion: DualQuaternion,
        point3: Vector3,
    ) -> DualQuaternion {
        let point_dual_quat = DualQuaternion {
            real: Quaternion::IDENTITY,
            dual: Quaternion::set(point3.x(), point3.y(), point3.z(), 0.0),
        };
    ...
    #[inline]
    pub fn linear_blending_dual_quat(
    ...
    // Not 100 percent sure implementation below is valid nor invalid. Use with
    // caution :)
    ...
    pub fn pow_dual_quaternion(dual_quaternion: DualQuaternion, exponent: f32) -> DualQuaternion {
        ...
        let se = (dual_quaternion.dual.to_pure() - s0 * half_d * (half_theta).cos())
            * rcp_half_theta_sin;

        let real = Quaternion::from_additive_form(exp_half_theta_cos, s0 * exp_half_theta_sin);

        let dual = Quaternion::from_additive_form(
            -exponent * half_d * exp_half_theta_sin,
    ...
    #[inline]
    pub fn screw_linear_blending_dual_quat(
        start: DualQuaternion,
        end: DualQuaternion,
        time: f32,
    ) -> DualQuaternion {
