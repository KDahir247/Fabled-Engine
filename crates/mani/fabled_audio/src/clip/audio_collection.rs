...
pub struct AudioCollection<T> {
    audio_collection_output: ambisonic::rodio::queue::SourcesQueueOutput<T>,
    audio_collection_input: std::sync::Arc<ambisonic::rodio::queue::SourcesQueueInput<T>>,
}
    ...
    T: ambisonic::rodio::Sample + Send,
{
    pub fn new(keep_alive: bool) -> Self {
        let (queue_input, queue_output) = ambisonic::rodio::queue::queue(keep_alive);
...
mod audio_collection_test {
    ...
    fn audio_collection() {
        ...
        let first_clip = RawClip::new(ambisonic::rodio::source::SineWave::new(28000))
            .take_duration(5, 0, FadeFilter::FADE);

        let second_clip = RawClip::new(ambisonic::rodio::source::SineWave::new(1100))
