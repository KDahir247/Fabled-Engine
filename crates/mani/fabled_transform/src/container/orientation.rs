// todo we need a clear distinction between local and global transformations
...
impl Orientation {
    ...
    // todo remove from_rotation_x,y,z
    ...
    //
    // let (_rotation_matrix, target_translation) =
    // transformation_matrix.split_at_mut(12);
    //
...
mod orientation_test {
    //  use crate::{Orientation, Scale, Transform};
    //
    // #[test]
    ...
    // glam::Quat::from_array(rotation).to_euler(glam::EulerRot::XYZ);
    //
    ...
    //
    // assert!(((orientation.forward[0] * 10.0).round() /
    // 10.0).eq(&proven_forward[0])); assert!(((orientation.forward[1] *
    // 10.0).round() / 10.0).eq(&proven_forward[1])); assert!(((orientation.
    // forward[2] * 10.0).round() / 10.0).eq(&proven_forward[2]));
    //
    //
    // assert!(((orientation.right[0] * 10.0).round() /
    // 10.0).eq(&proven_right[0])); assert!(((orientation.right[1] *
    // 10.0).round() / 10.0).eq(&proven_right[1])); assert!(((orientation.
    // right[2] * 10.0).round() / 10.0).eq(&proven_right[2])); }
    //
    // #[test]
